...
| when                    | `conversion-year`                                                                                                                               | `cie-conversion-year`                                                                                  | identical wherever a source exists; 0 elsewhere                                    |
...
| mineral soil            | inside `soil-emissions:tco2e-per-ha:{span}` ×4, as SOC × 44/12 × cropland loss fraction for a cropland destination, 0 for a pasture destination | `cie-mineral-soil-carbon-at-risk:tco2e-per-ha`: SOC × 44/12                                            | no loss fraction; not gated on destination; 0 on peat, as before                   |
| peat conversion         | inside `soil-emissions:tco2e-per-ha:{span}` ×4, as the 621 tCO2e/ha pulse replacing mineral soil on peat                                        | `cie-peat-transformation-emissions-undiscounted:tco2e-per-ha`                                          | split out from mineral soil; not gated on destination                              |
