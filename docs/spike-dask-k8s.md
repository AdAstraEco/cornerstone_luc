...
**Status:** proposal / scaffolding on branch `spike/dask-k8s`. Nothing is deployed. This document is the plan; the runnable stubs live in [`spike/`](../spike/).
...
- **One image** ([`spike/Dockerfile`](../spike/Dockerfile)) built from `uv.lock`, so the pod resolves exactly what a laptop run resolves.
- **A Job template** ([`spike/k8s/tile-job.yaml`](../spike/k8s/tile-job.yaml)) rendered once per tile by [`spike/run-tiles.py`](../spike/run-tiles.py), which resolves the tiles a country touches and `kubectl apply`s one Job each. `restartPolicy: Never`, `backoffLimit: 1`, `ttlSecondsAfterFinished` for self-cleanup.
...
- **GCS access via Workload Identity**: pods run as an existing KSA (already annotated to the GSA the Celery workers use). **No new IAM.** Cluster-specific names live in `spike/infra.env` (see `spike/infra.env.example`), not in tracked files.
...
An earlier draft of this spike stood up a remote Dask scheduler + worker Deployments and submitted tiles to it. That works, but for *this* goal it is the wrong instrument: it drags in evaluating and operating Dask itself (version skew between driver and workers, a shared scheduler a fat tile can destabilize, a `storage.py` change to reach the remote scheduler) — none of which is the question. A Dask-operational failure would masquerade as "the repo doesn't work in our infra." One Job per tile is a strict subset of that machinery minus the scheduler and minus the code change; failures isolate to a single tile's Job. Choosing a real production distribution layer (Dask, Celery fan-out, Argo, …) is a **follow-up**, informed by this spike but out of its scope.

## Known integration points / risks to verify in the spike
...
- **`.env` is a file, not env.** `Config.from_dot_env()` reads a `.env` file, so pods mount one from a Secret (`spike/k8s/README.md`). Pre-ingest HND once so workers hit the `SCRATCH_ROOT` cache and don't need the USDA/Harvard API keys.
...
- Choosing the **production** distribution layer (Dask cluster, Celery fan-out, Argo Workflows, …). This spike proves the repo runs and scales in our infra; how we'd operate that at production scale is a follow-up it informs.
